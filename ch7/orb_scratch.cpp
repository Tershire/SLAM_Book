...
// bash execute command: ch7$ build/orb_scratch
...
// FUNCTION PROTOTYPE /////////////////////////////////////////////////////////
void compute_descriptor(const cv::Mat &img, vector<cv::KeyPoint> &keypoints, 
                                            vector<DescType>     &descriptors);

void brute_force_match(const vector<DescType> &descriptors_1, 
                       const vector<DescType> &descriptors_2, 
...
int main(int argc, char **argv)
{
    ...
    // Extract FAST features (threshold=40) -----------------------------------
    // 1
    vector<cv::KeyPoint> keypoints_1;
    cv::FAST(img_1, keypoints_1, 40);
    vector<DescType> descriptors_1;
    compute_descriptor(img_1, keypoints_1, descriptors_1);

    // 2
    vector<cv::KeyPoint> keypoints_2;
    cv::FAST(img_2, keypoints_2, 40);
    vector<DescType> descriptors_2;
    compute_descriptor(img_2, keypoints_2, descriptors_2);
    ...
    // Hamming Distance -------------------------------------------------------
    vector<cv::DMatch> matches;
    brute_force_match(descriptors_1, descriptors_2, matches);
    ...
    cv::imwrite("orb_scratch_match.png", img_match);
    cv::waitKey(0);
...
// FUNCTION ///////////////////////////////////////////////////////////////////
// compute descriptor ---------------------------------------------------------
// don't know how it works, and half_... variable names are not clear
void compute_descriptor(const cv::Mat &img, vector<cv::KeyPoint> &keypoints, 
                                            vector<DescType>     &descriptors)
{
    ...
    for (auto &kp: keypoints) 
    {
        ...
        // angle = arctan(m01/m10)
        float m_sqrt = sqrt(m01 * m01 + m10 * m10) + 1e-18; // avoid div by 0?
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            {
                int idx_pq = i * 32 + k;
                cv::Point2f p(ORB_pattern[idx_pq * 4], 
                              ORB_pattern[idx_pq * 4 + 1]);
                cv::Point2f q(ORB_pattern[idx_pq * 4 + 2], 
                              ORB_pattern[idx_pq * 4 + 3]);
    ...
    cout << "bad / total: " << num_bad_points << "/" << keypoints.size() 
         << endl;
}
                             ...
                             vector<cv::DMatch> &matches)
{
    ...
    for (size_t i = 0; i < descriptors_1.size(); ++i)
    {
        ...
        for (size_t j = 0; j < descriptors_2.size(); ++j)
        {
            ...
            for (int k = 0; k < 8; k++)
            {
                distance += _mm_popcnt_u32(descriptors_1[i][k] ^
                                           descriptors_2[j][k]);
            }
            if (distance < MAX_DISTANCE && distance < m.distance)
            {
                m.distance = distance;
                m.trainIdx = j;
            }
